...
let rec dropSandB (target:option<Item>[,]) (x ,y) adj =
    let sizey, sizex = Array2D.sizes target
    match target[y + 1, x] with
    | None -> dropSandB target (x, y + 1) adj
    | Some _ ->
        if x = 0 then
            let target = Array2D.init (sizey + 1) (sizex + 2) (fun y x -> if y = sizey then Some Rock else if x = 0 then None else target[y, x - 1])
            dropSandB target (1, y) (adj + 1)
        else
            match target[y + 1, x - 1] with
            | None -> dropSandB target (x - 1, y + 1) adj
            | Some _ ->
                if x = sizex then
                    let target = Array2D.init (sizey + 1) (sizex + 2) (fun y x -> if y = sizey then Some Rock else if x > sizex then None  else target[y, x])
                    dropSandB target (x, y) adj
                else
                    match target[y + 1, x + 1] with
                    | None -> dropSandB target (x + 1, y + 1) adj
                    | Some _ -> (x, y), target, adj
...
let log (screen:option<Item>[,]) =
    let size,_ = Array2D.sizes screen
    [for i in 0..size  do
        let str = screen[i,*] |> Seq.map logItem |> Seq.reduce (+)
        Console.WriteLine(str)
        ] |> ignore

let rec sandFlowB startPoint src iter =
    match dropSandB src startPoint 0 with
    | p, arr, _ when p = startPoint ->
        log arr
        iter
    | (x, y), arr, adj->
        let (stx, sty) = startPoint
        Array2D.set arr y x (Some Sand)

        sandFlowB (stx + adj, sty) arr (iter + 1)
